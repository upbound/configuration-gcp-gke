...
uptest: $(UPTEST) $(KUBECTL) $(KUTTL)
	...
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e examples/gke-xr.yaml,examples/network-xr.yaml --setup-script=test/setup.sh --default-timeout=2400 || $(FAIL)
...
render:
	crossplane beta render examples/gke-xr.yaml apis/composition.yaml examples/functions.yaml -r
